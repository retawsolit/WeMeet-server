...
func (ac *AuthController) HandleVerifyToken(c *fiber.Ctx) error {
	...
	rr, roomDbInfo, rInfo, meta := ac.RoomModel.IsRoomActive(c.UserContext(), &wemeet.IsRoomActiveReq{
	...
	res := &wemeet.VerifyTokenRes{
		...
		RoomId:        &rId,
		UserId:        &uId,
		NatsSubjects: &wemeet.NatsSubjects{
...
func (ac *AuthController) HandleVerifyHeaderToken(c *fiber.Ctx) error {
	...
	claims, err := ac.AuthModel.VerifyWeMeetAccessToken(authToken, true)
	if err != nil {
		_ = c.SendStatus(errStatus)
