...
func (m *ExMediaModel) endPlayBack(req *wemeet.ExternalMediaPlayerReq) error {
	active := new(bool)
...
func (m *ExMediaModel) updateRoomMetadata(roomId string, opts *updateRoomMetadataOpts) error {
	...
	d := &wemeet.AnalyticsDataMsg{
		...
		EventName: wemeet.AnalyticsEvents_ANALYTICS_EVENT_ROOM_EXTERNAL_MEDIA_PLAYER_STATUS,
		RoomId:    roomId,
		HsetValue: &val,
	}
	if !roomMeta.RoomFeatures.ExternalMediaPlayerFeatures.IsActive {
		val = wemeet.AnalyticsStatus_ANALYTICS_STATUS_ENDED.String()
		d.EventName = wemeet.AnalyticsEvents_ANALYTICS_EVENT_ROOM_EXTERNAL_MEDIA_PLAYER_STATUS
		d.HsetValue = &val
	}
